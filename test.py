...
class Browser:
    ...
    def __init__(self, mobile: bool) -> None:
        ...
        self.proxy = CONFIG.browser.proxy  # Proxy setting from config
    ...
    def browserSetup(self) -> undetected_chromedriver.Chrome:
        ...
        if self.proxy:
            ...
            if parsed_proxy.scheme in ["http", "https"]:
                ...
                seleniumwireOptions["proxy"] = {
                    "http": f"socks5://{parsed_proxy.hostname}:{parsed_proxy.port}",
                    "https": f"socks5://{parsed_proxy.hostname}:{parsed_proxy.port}",
                    "no_proxy": "localhost,127.0.0.1",
                }
                if parsed_proxy.username and parsed_proxy.password:
                    seleniumwireOptions["proxy"]["auth"] = (parsed_proxy.username, parsed_proxy.password)
